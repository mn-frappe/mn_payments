...
class EbarimtClient:
    ...
    def __init__(self, endpoint: str = None, pos_no: str = None, merchant_tin: str = None, save_to_db: bool = True, send_email: bool = True, settings=None):
        ...
        self.session.headers.update({'Content-Type': 'application/json', 'User-Agent': 'EbarimtPythonSDK/1.0'})
    ...
    def create_receipt(self, request: CreateReceiptRequest) -> ReceiptResponse:
        ...
        if TaxType.NOT_VAT in receipts_by_tax:
            no_vat_request = self._build_request({TaxType.NOT_VAT: receipts_by_tax[TaxType.NOT_VAT]}, request, receipt_type, customer_tin)
        ...
        if receipts_by_tax:
            main_request = self._build_request(receipts_by_tax, request, receipt_type, customer_tin)
    ...
    def _build_receipt_items(self, items: List[ReceiptItem], merchant_info: MerchantInfo) -> Dict[TaxType, Dict[str, Any]]:
        ...
        for item in items:
            ...
            if tax_type not in receipts_by_tax:
                receipts_by_tax[tax_type] = {"items": [], "total_amount": 0.0, "total_vat": 0.0, "total_city_tax": 0.0}
    ...
    def _build_request(
        self, receipts_by_tax: Dict[TaxType, Dict[str, Any]], request: CreateReceiptRequest, receipt_type: ReceiptType, customer_tin: str
    ) -> Dict[str, Any]:
    ...
    def _send_email(self, response: ReceiptResponse, email_to: str):
        ...
        try:
            ...
            message = frappe.render_template("mn_payments/templates/emails/ebarimt_receipt.html", context)

            # Send email immediately
            frappe.sendmail(recipients=[email_to], subject=subject, message=message, header=["Tax Receipt", "green"], now=True)
    ...
    def _generate_qr_code_file(self, qr_data: str, bill_id: str):
        ...
        try:
            ...
            file_doc = frappe.get_doc({"doctype": "File", "file_name": f"ebarimt_qr_{bill_id}.png", "content": buffer.read(), "is_private": 0})
